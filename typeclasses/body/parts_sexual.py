...
@dataclass
class SexualPartState:
    """
    Runtime state for a sexual part on a character.
    Stored per-character, tracks current condition.
    """
    
    part_key: str
    
    # === SIZE (can differ from default) ===
    size: str = "average"
    ...
    cum_stored: float = 1.0          # Percentage full
    
    # === GENERAL ===
    sensitivity_modifier: float = 1.0  # Multiplier
    is_pierced: bool = False
    piercing_type: str = ""
    
    # === VIRGINITY TRACKING ===
    virgin_lost_to: str = ""         # Character dbref
    virgin_lost_when: float = 0.0    # Game timestamp
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "size": self.size,
            "orifice_state": self.orifice_state.value if isinstance(self.orifice_state, OrificeState) else self.orifice_state,
            "current_stretch": self.current_stretch,
            "current_wetness": self.current_wetness,
            "times_used": self.times_used,
            "largest_taken": self.largest_taken,
            "arousal": self.arousal.value if isinstance(self.arousal, ArousalState) else self.arousal,
            "knot_state": self.knot_state.value if isinstance(self.knot_state, KnotState) else self.knot_state,
            "cum_stored": self.cum_stored,
            "sensitivity_modifier": self.sensitivity_modifier,
            "is_pierced": self.is_pierced,
            "piercing_type": self.piercing_type,
            "virgin_lost_to": self.virgin_lost_to,
            "virgin_lost_when": self.virgin_lost_when,
        }
    
    @classmethod
    def from_dict(cls, data: dict) -> "SexualPartState":
        """Deserialize from dictionary."""
        state = cls(part_key=data.get("part_key", ""))
        state.size = data.get("size", "average")
        
        orifice_val = data.get("orifice_state", "virgin")
        state.orifice_state = OrificeState(orifice_val) if isinstance(orifice_val, str) else orifice_val
        ...
        arousal_val = data.get("arousal", "soft")
        state.arousal = ArousalState(arousal_val) if isinstance(arousal_val, str) else arousal_val
        
        knot_val = data.get("knot_state", "deflated")
        state.knot_state = KnotState(knot_val) if isinstance(knot_val, str) else knot_val
        
        state.cum_stored = data.get("cum_stored", 1.0)
        state.sensitivity_modifier = data.get("sensitivity_modifier", 1.0)
        state.is_pierced = data.get("is_pierced", False)
        state.piercing_type = data.get("piercing_type", "")
