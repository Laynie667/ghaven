...
def calculate_fit(
    ...
    Args:
        ...
        orifice_state: Current state of the orifice
    ...
    if orifice_state.orifice_state == OrificeState.VIRGIN:
        base_girth_cap *= (1.0 - VIRGIN_TIGHTNESS_BONUS)
    elif orifice_state.orifice_state in (OrificeState.RELAXED, OrificeState.LOOSE):
        base_girth_cap *= (1.0 + RELAXATION_BONUS)
    
    # Adjust for current stretch (if already stretched)
    base_girth_cap += orifice_state.current_stretch
...
def calculate_knot_lock(
    ...
    effective_cap += orifice_state.current_stretch
...
def check_virginity_loss(
    ...
    Args:
        orifice_state: Current orifice state
